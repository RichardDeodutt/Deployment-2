...
aptinstalllog(){
    ...
    Pkg=$1
    #Install using apt-get then log if it fails or not
    apt-get install $Pkg -y && log "$(printokay "Successfully installed $Pkg")" || log "$(printerror "Failure installing $Pkg")"
}

installjenkins(){
    installjenkins.sh && log "$(printokay "Successfully installed jenkins")" || log "$(printerror "Failure installing jenkins")"
}
...
main(){
    ...
    git clone $RepositoryURL && log "$(printokay "Successfully cloned $Pkg")" || log "$(printerror "Failure cloning $Pkg")"
    #Install jenkins
    installjenkins
